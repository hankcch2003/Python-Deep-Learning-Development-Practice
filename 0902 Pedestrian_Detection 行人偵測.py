...
# 建立行人影片的讀取物件
cap_pedestrian = cv2.VideoCapture(pedestrian_video)

# 若行人影片成功開啟，持續讀取行人影片的每一幀畫面
while cap_pedestrian.isOpened():

    # 從攝影機讀取一幀影像，ret表示讀取成功與否，frame為影像陣列
    ret, frame = cap_pedestrian.read()

    # 檢查是否成功讀取行人影片的影格，若讀取失敗則跳出迴圈
    if not ret:
        print("無法讀取行人影片")
        break
    ...
    # 逐一畫出每個偵測到的行人區域，並在畫面上以黃色矩形框標示行人位置，並設定線寬為2
