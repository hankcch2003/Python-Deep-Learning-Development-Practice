...
# 建立車輛影片的讀取物件
cap_car = cv2.VideoCapture(car_video)

# 若車輛影片成功開啟，持續讀取車輛影片的每一幀畫面
while cap_car.isOpened():
    # 從攝影機讀取一幀影像，ret表示讀取成功與否，frame為影像陣列
    ret, frame = cap_car.read()

    # 檢查是否成功讀取車輛影片的影格，若讀取失敗則跳出迴圈
    if not ret:
        print("無法讀取車輛影片")
        break
    ...
    # 逐一畫出每個偵測到的車輛區域，並在畫面上以黃色矩形框標示車輛位置，並設定線寬為2
    for (x, y, w, h) in cars:
